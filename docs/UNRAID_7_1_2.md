...
unzip /tmp/Kitchen_Planner_0.6.4.7.zip
cp .env.example .env
nano .env
chmod +x unraid.sh
./unraid.sh install
...
3. Extract the 0.6.4.7 ZIP over the source directory. It contains `.env.example`, not `.env`, so existing secrets are preserved.
...
The update applies append-only migrations through `0016_multibuy_flyer_price_integrity.sql`; 0.6.4.7 requires no new database migration. It removes household-specific defaults and examples from distributable source, loads login choices from PostgreSQL, and preserves reviewed changes to automatic shopping rows when a draft weekly-plan revision is saved. Deterministic reconciliation still enforces required minimum shopping coverage. The updater keeps the Settings, package, Compose and installer version values aligned, waits for the replacement Kitchen Planner container to report healthy, removes the exact superseded Kitchen Planner image, and prunes only unused Docker build cache older than 24 hours. The previous image is retained when health validation fails. Existing household members, PIN hashes, inventory, preferences, recipes, flyers, drafts, committed meals, shopping, feedback and audit history are preserved; no workbook cutover is required.

## Upgrade from 0.4.0 or 0.3.0

1. Run `./unraid.sh backup` in the current source directory.
2. Extract the 0.6.4.7 ZIP over the source directory. It contains `.env.example`, not `.env`, so existing secrets are preserved.
3. Run:
