...
unzip /tmp/Kitchen_Planner_0.6.4.8.zip
cp .env.example .env
nano .env
chmod +x unraid.sh
./unraid.sh install
...
3. Extract the 0.6.4.8 ZIP over the source directory. It contains `.env.example`, not `.env`, so existing secrets are preserved.
