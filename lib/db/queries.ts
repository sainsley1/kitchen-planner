...
export async function listFlyers(householdId:string):Promise<FlyerRecord[]>{const result=await poolOrThrow().query<Omit<FlyerRecord,"sales">&{sales:FlyerSaleRecord[]}>(`SELECT f.id,f.store_name AS "storeName",f.store_location AS "storeLocation",f.valid_from::text AS "validFrom",f.valid_until::text AS "validUntil",f.source_type AS "sourceType",f.source_url AS "sourceUrl",f.original_filename AS "originalFilename",f.mime_type AS "mimeType",(f.storage_path IS NOT NULL) AS "hasFile",f.status,f.extraction_warnings AS "extractionWarnings",f.created_at::text AS "createdAt",f.committed_at::text AS "committedAt",COALESCE((SELECT jsonb_agg(jsonb_build_object('id',s.id,'item',s.item,'brand',s.brand,'category',s.category,'packageSize',s.package_size,'price',s.price::text,'regularPrice',s.regular_price::text,'savingsAmount',s.savings_amount::text,'discountPercent',s.discount_percent::text,'pricingUnit',s.pricing_unit,'multiBuyQuantity',s.multi_buy_quantity,'memberOnly',s.member_only,'limitText',s.limit_text,'notes',s.notes,'confidence',s.confidence::text,'evidenceText',s.evidence_text,'sourceReference',s.source_reference,'status',s.status,'prioritized',s.prioritized,'normalizedUnitPrice',s.normalized_unit_price::text,'normalizedUnitMeasure',s.normalized_unit_measure,'estimatedRegularPrice',s.estimated_regular_price::text,'dealGrade',s.deal_grade) ORDER BY s.prioritized DESC,CASE s.status WHEN 'proposed' THEN 0 WHEN 'accepted' THEN 1 ELSE 2 END,lower(s.item)) FROM flyer_sale_items s WHERE s.flyer_source_id=f.id),'[]'::jsonb) AS sales FROM flyer_sources f WHERE f.household_id=$1 AND f.archived_at IS NULL ORDER BY CASE f.status WHEN 'review' THEN 0 ELSE 1 END,f.valid_until DESC,f.created_at DESC`,[householdId]);return result.rows;}
