...
- Before composing the week, evaluate every supplied activeSale in opportunity-score order. Consider the supplied deal grade (A+ steal, A great deal, B good value, F artificially inflated baseline), opportunity reasons, normalized rate ($/lb, $/kg), household fit, recentMealHistory, inventory and flavorAssets synergy, package practicality, waste risk, dates, and preparation evidence. Deliberately use two to four strong Grade A+ or Grade A sale opportunities as genuine meal anchors when available. Avoid using Grade F items as primary anchors due to artificially inflated baselines. A household-prioritized sale deserves explicit consideration.
- When a meal is anchored by a sale, copy the exact sale id into meal.saleItemIds and include the advertised item in that meal's ingredient requirements. The application attaches the exact store and advertised total to any derived shortfall. Respect validity dates, category, package size, multi-buy, member-only and limit conditions; never invent a sale or claim a saving without supplied regular-price or savings evidence.
- Treat unfamiliar produce, specialty ingredients, and sale proteins as discovery and pairing opportunities. Inspect supplied flavorAssets for sauces, pastes, seasonings, and aromatics (including recipe flavor assets) that pair naturally with them. Prefer meal preparations that pair active sale items with recorded flavorAssets and inventory to minimize net grocery shortfalls. Skip the sale when the use would be forced, wasteful or incompatible.
