...
Version 0.6.4.7 removes household-specific names and preference examples from distributable source and loads the login member list from PostgreSQL. Existing household users, PIN hashes, inventory, preferences and planning history remain unchanged during an upgrade. It also retains the 0.6.4.6 fix that preserves reviewed edits to automatically generated shopping rows in a draft weekly plan.

## 0.6.4.7 workflows

0.6.4.7 retains the complete 0.6.4.6 workflow, including editable draft shopping rows, bounded targeted-workflow recovery, warning persistence, conversion-aware inventory reconciliation, resilient multi-buy flyer extraction, ranked sale opportunities, ingredient-aware planning, multiple same-day prep items, selectable Balanced/Deep weekly planning, per-run cost visibility, direct-use cold-storage planning, grocery-restock repair, proposal archival, meal-day inventory review, Docker storage safeguards and the household recipe library.
...
- **Job controls:** cancel a queued or running full-week job, retry a failed or cancelled job, and inspect elapsed time, current stage, model/search activity and errors. A cancelled provider response is prevented from creating a late draft.
